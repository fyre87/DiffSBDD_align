...
from dataset_align import ProcessedLigandPocketDataset
import utils
...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        ##** CHANGED
        # self.ref_ddpm = torch.load(ref_path, map_location=self.device)
        # self.ref_ddpm.load_state_dict(self.ref_ddpm['model'])
        self.ref_model = None
    ...
    def __getstate__(self):
        # We don't want to save the ref model parameters. So this is overriding
        # the getstate method so that we don't save them. 
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == 'fit':
            self.train_dataset = ProcessedLigandPocketDataset(
                Path(self.datadir, 'train.npz'), transform=self.data_transform, good_fraction = self.good_fraction)
            self.val_dataset = ProcessedLigandPocketDataset(
                Path(self.datadir, 'val.npz'), transform=self.data_transform, good_fraction = self.good_fraction)
        elif stage == 'test':
            self.test_dataset = ProcessedLigandPocketDataset(
                Path(self.datadir, 'test.npz'), transform=self.data_transform, good_fraction = self.good_fraction)
        else:
    ...
    def get_ligand_and_pocket(self, data):
        ##** Added this failsafe
        # if data['num_lig_atoms'].dim() == 0:
        #     data['num_lig_atoms'] = data['num_lig_atoms'].unsqueeze(0)
        ligand = {
        ...
        ##** Added this failsafe
        # if data['num_pocket_nodes'].dim() == 0:
        #     data['num_pocket_nodes'] = data['num_pocket_nodes'].unsqueeze(0)
        pocket = {
    ...
    def forward(self, data):
        # print(data)
        # print("Lig coords: ", data['lig_coords'].size())
        # print("Lig one hot: ", data['lig_one_hot'].size())
        # print("Lig mask: ", data['lig_mask'].size())
        # print("num_lig_atoms: ", data['num_lig_atoms'].size())
        # print("pocket_coords: ", data['pocket_coords'].size())
        # print("pocket_one_hot: ", data['pocket_one_hot'].size())
        # print("pocket_mask: ", data['pocket_mask'].size())
        # print("num_pocket_nodes: ", data['num_pocket_nodes'].size())
        # raise ValueError("Done")
        # Data is a dict with keys: dict_keys(['names', 'receptors', 'lig_coords', 'lig_one_hot', 'lig_mask', 'num_lig_atoms', 'pocket_coords', 'pocket_one_hot', 'pocket_mask', 'num_pocket_nodes'])
        ligand, pocket = self.get_ligand_and_pocket(data)
        # print("SELF DEVICE: ", self.device)

        # print("ligand: ", ligand)
        # print("pocket: ", pocket)
        # print("ligand['x'].shape(): ", ligand['x'])
        # print("ligand['one_hot'].shape(): ", ligand['one_hot'].size())
        # print("ligand['size'].shape(): ", ligand['size'])
        # print("ligand['mask'].shape(): ", ligand['mask'].size())
        # print("pocket['x'].shape(): ", pocket['x'])
        # print("pocket['one_hot'].shape(): ", pocket['one_hot'].size())
        # print("pocket['size'].shape(): ", pocket['size'])
        # print("pocket['mask'].shape(): ", pocket['mask'].size())

        # Ligand is a dict with keys: dict_keys(['x', 'one_hot', 'size', 'mask'])
        # Pocket is a dict with keys: dict_keys(['x', 'one_hot', 'size', 'mask'])

        # print("Ligand x shape: ", ligand['x'].size())
        # print("Ligand one_hot shape: ", ligand['one_hot'].size())
        # print("Ligand size shape: ", ligand['size'].size())
        # print("Ligand mask shape: ", ligand['mask'].size())
        # print("Pocket x shape: ", pocket['x'].size())
        # print("Pocket one_hot shape: ", pocket['one_hot'].size())
        # print("Pocket size shape: ", pocket['size'].size())
        # print("Pocket mask shape: ", pocket['mask'].size())
        # raise ValueError("hi")
    ...
    def training_step(self, data, *args):
        ...
        try:
            ...
            ##** CHANGED
            with torch.no_grad():
                ref_nll, ref_info = self.ref_model.forward(data)
            ...
            # print("info: ", info)
            good_nll, bad_nll = nll.chunk(2) # [N,l]
            ref_good_nll, ref_bad_nll = ref_nll.chunk(2) # [N,l]

            # print("Loss 1: ", good_nll)
            # print("Loss 2: ", bad_nll)
            # print("Ref Loss 1: ", ref_good_nll)
            # print("Ref Loss 2: ", ref_bad_nll)
            # raise ValueError("Done")
            
            # model_diff = loss_1 - loss_2 # [N,l]
            # ref_diff = ref_loss_1 - ref_loss_2 # [N,l]
        ...
        # inside_term = - beta_dpo * (model_diff - ref_diff) # - alpha_dpo * margin_rewards.unsqueeze(1)  # [N,L]
        # energy_loss = -1 * F.logsigmoid(inside_term).sum(dim=1) / inside_term.size(1) # [N]

        losses = -1 * F.logsigmoid(beta_dpo * (model_diff - ref_diff)) 
    ...
    def print_params(self):
        ...
        for param, value in vars(self).items():
            # if printout:
            #     print(f"{param}: {type(value)}")
            if param == "eval_params":
                print(f"{param}: {value}")
                print("HELLO")
            # if param != "eval_params":
            #     print(f"{param}: {value}")
            # else:
            #     "Not printing out:::", param
            if param == "eval_params":
                printout = True
